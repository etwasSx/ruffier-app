...
--------------------------------------------
# Описание модуля seconds.py

Этот модуль определяет класс `Seconds`, наследующий от `Label` из библиотеки `Kivy`, который создает текстовый таймер с отсчетом времени в секундах. 

## Атрибуты класса

- **`done`**: `BooleanProperty`
  - Свойство, показывающее состояние таймера: `True`, если таймер завершен, и `False` — если нет.
  
## Конструктор `__init__(self, total, **kwargs)`

- **Параметры**:
  - `total` — общее количество секунд для отсчета.
  - `**kwargs` — дополнительные аргументы, передаваемые в родительский класс `Label`.

- **Описание**:
  - Инициализирует таймер, устанавливая начальное значение текущего времени (`self.current`) в `0`, а `self.total` — в заданное значение `total`. 
  - Устанавливает текст метки в формате `"Прошло секунд: 0"`.

## Методы

- **`restart(self, total, **kwargs)`**
  - Сбрасывает таймер на новое значение `total`, устанавливает `self.current` в `0`, обновляет текст метки и запускает таймер заново.
  
- **`start(self)`**
  - Запускает таймер, который обновляет значение каждую секунду, вызывая метод `change`.

- **`change(self, dt)`**
  - Увеличивает текущее время `self.current` на `1` и обновляет текст метки.
  - Когда текущее время (`self.current`) достигает или превышает `total`, завершает таймер, устанавливает `done` в `True` и останавливает дальнейшие обновления.
